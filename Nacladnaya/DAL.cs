    ...
    public class DAL
    {
        public static OrderEntity GetOrderById(int OrderId)
        {
            ...
            string cmdText = "SELECT Заказы.NППЗаказа as OrderId," +
                             ...
                             "INNER JOIN НомераЦехов ON Заказы.Заказчик = НомераЦехов.NЦеха " +
            ...
            try
            {
                ...
                if (reader.Read())
                {
                    ...
                    if (order.OrderNumber.StartsWith("23-") || order.OrderNumber.StartsWith("24"))
                    ...
                    order.Customer = (string)reader["Customer"];
